...
contract FileStorage {
    ...
    */
    ...
    event FilenameChanged(string filename, string newName);
    ...
    modifier onlyFileOwner(string memory _filename) {
        require(msg.sender == files[_filename].fileOwner, "Only the file owner can do this");
        _;
    }
    ...
    function renameFile(string memory _oldName, string memory _newName) external onlyFileOwner(_oldName) fileExists(_oldName) {
        ...
        emit FilenameChanged(_oldName, _newName);
    }
